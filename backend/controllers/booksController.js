	...
	upload.fields([
		...
		if (err) {
			...
			try {
				...
				res
					.status(500)
					.json({ success: false, message: "Failed to create book", error });
			}
		}
	});
};
...
exports.updateBook = (req, res) => {
	upload.fields([
		...
		if (err) {
			...
			var PDFLinkUp =
				req.files && req.files.pdfVersion && req.files.pdfVersion[0];
			...
			try {
				const cover = coverUp
					? await cloudinary.uploader.upload(coverUp.path, {
							folder: "images",
					  })
					: null;

				const PDFLink = PDFLinkUp
					? await cloudinary.uploader.upload(PDFLinkUp.path, {
							folder: "pdfs",
					  })
					: null;
